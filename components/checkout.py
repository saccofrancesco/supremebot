    ...
    def __init__(self) -> None:
        ...
        if self.country in [country for country in ZONES.keys()]:
            ...
            self.zone: str = zone_col.selectbox(
                " ",
                country_zones(self.country),
                label_visibility="collapsed",
                index=(
                    country_zones(self.country).index(get_pay_prop("zone"))
                    if pay_config and get_pay_prop("zone") != "None"
                    else 0
                ),
            )
        else:
        ...
        self.expiration_year: str = expiration_year_col.selectbox(
            ...
            index=(
                get_card_exp_years().index(get_pay_prop("expiration_year"))
                if pay_config
                else 0
            ),
        )
        ...
        self.expiration_month: str = expiration_month_col.selectbox(
            ...
            index=(
                months_in_numbers(int(self.expiration_year)).index(
                    get_pay_prop("expiration_month")
                )
                if pay_config
                else 0
            ),
